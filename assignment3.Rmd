...
parcels_census_clean <- parcels_census %>%
...
lihtc_clean <- lihtc %>%
  select(project, proj_add, proj_cty, proj_zip, latitude, longitude, geoid10, n_units, li_units)
...
```{r, message=FALSE}
table_prop_value <- left_join(sf_1314_clean, sf_1819_clean) %>%
...
```{r, message=FALSE}
table_prop_parcels_census <- left_join(table_prop_value, parcels_census_clean)

table_prop_parcels_census <- table_prop_parcels_census %>%
  select(geoid10, block_lot, block_num, lot_num, TotValue_1314, TotValue_1819, pct_change, newpropval, street_name)
```

```{r}
table_prop_parcels_census$geoid10<-as.character(table_prop_parcels_census$geoid10)

table_prop_lihtc <- left_join(table_prop_parcels_census, lihtc_clean)
```

```{r, message=FALSE}
table_prop_lihtc <- table_prop_lihtc %>%
...
table_prop_lihtc_inc <- left_join(table_prop_lihtc, table_acs_income)

table_prop_lihtc_inc <- table_prop_lihtc_inc %>%
  mutate(pct_AMI = med_income_E / 143100) %>%
  mutate(pct_li_units = li_units / n_units)

table_data <- table_prop_lihtc_inc %>%
  select(geoid10, block_num, lot_num, TotValue_1314, TotValue_1819, pct_change, newpropval, med_income_E, pct_AMI, aff_housing, pct_li_units) 

data_clean <- table_data %>%
  filter(geoid10 != "NA")
```
...
This dataset includes 269,906 observations. 

# Descriptive Statistics

## Continuous Variables

The continuous variables we want to use are percent change of property value, income percent of AMI, and XXX.

### Sample Mean and 95-percent Confidence Interval
```{r}
#income_t_test <- t.test(table_data$AvgIncome)
...
incAMI_quartiles <- quantile(data_clean$pct_AMI, na.rm = TRUE)
```
